...
public class DependencyServiceImpl implements DependencyService {
  ...
  Dependency getNestedDependency(URI uri) {
    ...
    try {
      ...
      log.debug("unable to open nested jar manifest for {}", uri);
    }
    log.info(
  ...
  /**
   * Get Sqreen agent version from classloader
   *
   * @return Sqreen agent version, {@literal null} if unable to read it
   */
  public static String getSqreenAgentVersion() {

    String sqreenVersion = null;
    try {
      Enumeration<URL> manifests =
          DependencyServiceImpl.class.getClassLoader().getResources(JarFile.MANIFEST_NAME);
      while (sqreenVersion == null && manifests.hasMoreElements()) {
        try (InputStream input = manifests.nextElement().openStream()) {
          sqreenVersion = getSqreenManifestVersion(new Manifest(input));
        }
      }
    ...
    return sqreenVersion;
  }

  /**
   * Get Sqreen agent version from manifest
   *
   * @param manifest jar manifest
   * @return Sqreen agent version, null if manifest does not match Sqreen manifest
   */
  public static String getSqreenManifestVersion(Manifest manifest) {
    Attributes attributes = manifest.getMainAttributes();
    String name = attributes.getValue("Bundle-SymbolicName");
    String version = attributes.getValue("Bundle-Version");

    if (name == null || version == null || !name.equals("sqreen-agent")) {
      return null;
    } else {
      return version;
    }
